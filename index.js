...
export default class Trimmer extends React.Component {
  ...
  createTrackPanResponder = () => PanResponder.create({
    ...
    onStartShouldSetPanResponder: (evt,
                                   gestureState) => !this.state.scrubbing && !this.state.trimming,
    ...
    onPanResponderGrant: (evt, gestureState) => {
      ...
      if (touches.length == 2) {
        const pinchDistance = this.calculatePinchDistance(touches[0].pageX,
          touches[0].pageY,
          touches[1].pageX,
          touches[1].pageY);
    ...
    onPanResponderMove: (evt, gestureState) => {
      ...
      if (touches.length == 2) {
        ...
        const newBoundedTrackScaleValue = Math.max(Math.min(newTrackScaleValue,
          maximumZoomLevel), 1);
